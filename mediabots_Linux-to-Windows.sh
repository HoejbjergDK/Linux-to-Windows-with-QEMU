...
link1_status=$(curl -Is http://128.76.169.10/test/Windows.ISO | grep HTTP | cut -f2 -d" " | head -1)
link2_status=$(curl -Is http://128.76.169.10/test/Windows.ISO | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots https://archive.org/download/WS2012R2/WS2012R2.ISO # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2012R2.ISO http://128.76.169.10/test/Windows.ISO 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots http://128.76.169.10/test/Windows.ISO
else
